...
const HangOn = ({ color }) => (
  <Block>
    <InBl>
      Hang On, doesn't look like {color.toUpperCase()} is a valid hex color, let's try again.
    </InBl>
  </Block>
)
            ...
            matches => (
              ...
              >
                ...
                <Block width='100%' maxWidth={395} padding={16}>
                  <Input value={color} onChange={e => this.setState({ color: e.target.value }, this.toStorage)} />
                  {checkHex(color) ? <ColorBlock color={color} /> : <HangOn color={color} />}
                </Block>
                {checkHex(color) && <Scale fixed={matches} depth={matches ? 12 : 6} color={color} />}
              </Flex>
            )
        }
        </Media>
